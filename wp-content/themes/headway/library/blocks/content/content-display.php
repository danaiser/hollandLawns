...
class HeadwayContentBlockDisplay {
	...
	function display_comments($hook_args) {
		...
		/* Used to prepare $wp_query to display comments */
		comments_template();
					...
					echo '<h3 id="comments">';
						...
						/* Comments Area Responses Formatting */
							...
							if ( $comments_number == 1  ) 
								$comments_heading_responses_format = stripslashes($this->get_setting('comments-area-heading-responses-number-1', 'One Response'));
							else
								$comments_heading_responses_format = stripslashes($this->get_setting('comments-area-heading-responses-number', '%num% Responses'));
						...
						echo str_replace(array('%responses%', '%title%'), $comments_heading_replacements, $this->get_setting('comments-area-heading', '%responses% to <em>%title%</em>'));
	...
	function parse_meta($meta) {
		...
		foreach ( $variables as $variable ) {
			...
			switch ( $variable ) {
				...
				case 'tags':
					$replacement['tags'] = (get_the_tags() != NULL) ? get_the_tag_list('<span class="tag-links">','<span class="tag-sep">, </span>','</span>') : '';
				break;
